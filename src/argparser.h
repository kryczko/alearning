    ...
    public:
        ...
        po::variable_value operator[](const string option) {
            if (this->vm.count(option)) {
                ...
                cout << "ERROR: you tried accessing an arg that was not there.\n";
                exit(-1);
            }
        }
        ...
        ArgParser(int argc, char ** argv) {
            ...
            ("dopants", po::value<std::vector<string>>(), "Dopants type followed by number of dopant atoms to insert into lattice.")
            ("number", po::value<int>()->default_value(10000), "Number of configurations to generate.")
            ("etarget", po::value<int>()->default_value(1000000), "Target energy when sampling lattices.")
